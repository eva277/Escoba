...
namespace Cliente
{
    public partial class Cliente : Form
    {
            ...
            if (turno.Equals(idJugador))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (CountCardsTable() == 0)
                        {
                            escobas++;
                        }
                        string[] points = sr.ReadLine().Split('#');

                        escobas += Convert.ToInt16(points[0]);
                        numCartas += Convert.ToInt16(points[1]);
                        oros += Convert.ToInt16(points[2]);
                        sietes += Convert.ToInt16(points[3]);
                        sieteOros += Convert.ToInt16(points[4]);
        ...
        private int CountCardsPlayer()
        {
            num = 3;
            foreach (Button button in listButton)
            {
                if (button.BackgroundImage == null && button.Name.StartsWith("Player"))
                {
                    num--;
                }
            }
        ...
        private void CheckCardstimer_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                else if(num==0 && dato.Equals("GetPoints"))
                {
                    string puntos = escobas.ToString() + "/"
                        + numCartas.ToString() + "/"
                        + oros.ToString() + "/"
                        + sietes.ToString() + "/"
                        + sieteOros.ToString() + "/";
